...
int main(int argc, char* argv[])
{
   ...
   if (argc==1) {
      ...
      printf("All configuration are set by the SynSpec.cfg file.\n");
      printf("To plot sepctrum use: gnuplot SynSpec.plt SynSpec.dat\n");
   }
   
   config_t c;
   MakeConfig("SynSpec.cfg", &c);
   InitConfiguration(&c);
   ReadConfiguration("SynSpec.cfg", &c);
   ...
   for (i=0; i<=c.nbr_src-1; ++i) {
      AddSource(c.src_name[i],c.src_int[i],LineSpec,length,&udr_cnt,&ovr_cnt,LT);
   }
  ...
  // printf("detector resolution applied.\n");
   ...
   Normalize(DetSpec, length, c.norm, c.max);

   WriteOutput(DetSpec, length, LT);
      ...
      if(LineSpec[i_E].in > TH) {
         for (i=0; i<NBR_FLUO; ++i) {
            ...
            if (!strcmp(c.fluo_mat[i], "Ag")) {
               i_fluo = e2i(21990);
               if (i_E > i_fluo) {LineSpec[i_fluo].in += c.fluo_int[i]*0.53;}
               i_fluo = e2i(22163);
               if (i_E > i_fluo) {LineSpec[i_fluo].in += c.fluo_int[i];}
               continue;
            }
            ...
            if (!strcmp(c.fluo_mat[i], "Au")) {
               i_fluo = e2i(66990); //Ka2
               if (i_E > i_fluo) {LineSpec[i_fluo].in += c.fluo_int[i]*0.59;}
               i_fluo = e2i(68804); //Ka1
               if (i_E > i_fluo) {LineSpec[i_fluo].in += c.fluo_int[i];}
               //i_fluo = e2i(78274); //Kb123
               //if (i_E > i_fluo) {LineSpec[i_fluo].in += c.fluo_int[i]*0.40;}
               i_fluo = e2i(77580); //Kb3
               if (i_E > i_fluo) {LineSpec[i_fluo].in += c.fluo_int[i]*0.12;}
               i_fluo = e2i(77984); //Kb1
               if (i_E > i_fluo) {LineSpec[i_fluo].in += c.fluo_int[i]*0.23;}
               i_fluo = e2i(80150); //Kb2
               if (i_E > i_fluo) {LineSpec[i_fluo].in += c.fluo_int[i]*0.08;}
               continue;
            }
...
void ApplyEscape(spec_t* LineSpec, int length, config_t c)
{
   ...
   if (!strcmp(c.det_mat, "CdTe")) { 
      ...
      P_esc[0]   = 0.125*0.84; //fluo yield 0.84. todo: precise the 0.125
                               //idea was: 0.25 (out of thesis) / 2 because of
                               //the chance to hit Cd or Te
      i_E_edg[1] = e2i(31814);  
      i_E_esc[1] = e2i(27472); //Te-Ka
      P_esc[1]   = 0.075*0.88; //fluo yield 0.88. todo: precise the 0.075 
...
void Normalize(spec_t* DetSpec, int length, float norm, float max)
{
   ...
   for (i=0; i<=length-1; ++i) {
      if (DetSpec[i].in < max) DetSpec[i].in = 0.0;
   }
}
//------------------------------------------------------------------------------
...
{
   if (!strcmp("mono100", src)) {
      LineSpec[e2i( 100000)].in += src_int;
   } else {
      char file_name[64];
